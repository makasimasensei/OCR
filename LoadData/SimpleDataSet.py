...
from Preprocessing.DecodeImage import *
from Preprocessing.MakeBorderMap import *
from Preprocessing.MakeShrinkMap import *
from torch.utils.data import Dataset
from Preprocessing.image_preprocessing import Normalize_image


class SimpleDataSet(Dataset):
    def __init__(self, mode, label_file, data_dir, seed=None, augmentation=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode.lower() == "train":
            ...
            data = {'img_path': img_path, 'image': img_decode, 'box': boxes}

        generate_text_border = MakeBorderMap(shrink_ratio=0.4, thresh_min=0.3, thresh_max=0.7)
        data.update(generate_text_border(data, boxes))

        generate_shrink_map = MakeShrinkMap()
        data.update(generate_shrink_map(data, boxes))

        if self.augmentation is True:
            assert self.mode.lower() == 'train', "augmentation should not be used in eval mode"
    ...
    def apply_rotation(local_data):
        fixed_angle = random.randint(-30, 30)
        for key, value in local_data.items():
            if key == 'img_path' or key == 'box':
                continue
            elif key == 'image':
                ...
                rotated_tensor = torchvision.transforms.functional.rotate(new_tensor, angle=fixed_angle, expand=True)
...
if __name__ == "__main__":
    mode_global = "train"
    label_global = "E:/anaconda/envs/pytorch/graduation_project/train_data/Art/text_localization/train.txt"
    dir_global = "E:/anaconda/envs/pytorch/graduation_project/train_data/Art/text_localization/train_images"
    SimpleDataSet(mode_global, label_global, dir_global)
