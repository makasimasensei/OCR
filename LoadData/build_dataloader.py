...
from LoadData.SimpleDataSet import *


def collate_fn(batch):
    # 自定义转换操作
    # 这里以处理不定长序列为例
    img_path, image, img_norm, threshold_map, threshold_mask, shrink_map, shrink_mask, box = [], [], [], [], [], [], [], []
    for i in range(len(batch)):
        if 'box' in batch[i]:
            box.append(batch[i]['box'])
    ...
    data = {'image': image, 'img_path': img_path, 'img_norm': img_norm_cat,
            ...
            'shrink_map': shrink_map, 'shrink_mask': shrink_mask, 'box': box}

    return data


def build_dataloader(mode, label_file, data_dir, batch_size, num_workers, seed=None, augmentation=False):
    # 创建数据读取类
    dataset = SimpleDataSet(mode, label_file, data_dir, seed, augmentation)
